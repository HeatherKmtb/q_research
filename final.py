...
def grid(folderin, fileout, folderout, naming=4, eco_loc=2):
    """
    Function to compute q and provide results (csv) and figures (pdf)
...
def eco_hxcd(folderin, fileout, eco_loc=2):
    """
    Function to compute q and provide results (csv) and figures (pdf)
    ...
    #create df for results
    results = pd.DataFrame(columns = ['eco', 'height', 'cd', 'result', 'deg_free'])
    #resultsb = pd.DataFrame(columns = ['eco', 'ID', 'qout', 'r_sq', 'deg_free', 'rmse'])
    ...
    for file in fileList:
        ...
        results = results.append({'eco': eco, 'height': mean_h,
                                  'cd': mean_cd, 'result': result, 
                                  'deg_free': footprints}, ignore_index=True)
